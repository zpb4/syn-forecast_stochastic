...
for(i in 1:length(skill_mods)){
  ...
  hefs_ens_plt<-ggplot(df)+theme_minimal()+
    ...
    labs(x='forecast lead (days)',y='flow (kcfs)',title=TeX(tt))+
...
for(l in 1:length(lds)){
  ...
  ecrps_dt <- data.table(x=1,syn=shefs_mat,syncc=shefscc_mat)
  
  df <- melt(ecrps_dt,id='x')
  
  df$x <- rep(rep(c(1:length(skill_mods)),2),each=length(ecrps_dt$x))
  id_vec <- as.vector(df$variable)
  id_vec[str_detect(as.vector(df$variable),'syn')]<-1
  id_vec[str_detect(as.vector(df$variable),'syncc')]<-2
  df$grp <- factor(x=as.numeric(id_vec),levels=1:2,labels=c("shefs",'shefscc'))
...
ggsave(paste('e:/Projects/FIRO/firo_syn-forecast_stochastic-CC/figs/manuscript/combined_ens-plot_verification-plot_condensed_skillmods_fig5_evt=',evt_order,'_nn=',nn_order,'_lds=',str_flatten(leads,collapse='-'),'_samp=',samp_no,'_mods=',str_flatten(skill_mods,'-'),'.png',sep=''),comb_plot,dpi=320,width=9,height=8.5,unit='in')
