...
hefs_plts = vector('list',length(skill_mods))
ylm_scale = 2.1
show_x = F
labs = letters[1:length(skill_mods)]

for(i in 1:length(skill_mods)){
  hefs_plt_dt <- data.table(x=0:15,hefs=rbind(rep(obs_hefs[hind_evt_idx-lead],dim(hefs_fwd)[1]),hefs_ens_sset[i,,]),
  ...
  hefs_ens_plt<-ggplot(df)+theme_minimal()+
    ...
    annotate('text',x=lead,y=0.95*ylm,label=hind_evt_date,size=4,hjust=-0.25)+
...
mat<-matrix(1:length(skill_mods),ncol=length(skill_mods))
hefs_gplot<-marrangeGrob(hefs_plts,nrow=1,ncol=length(skill_mods),top='')
#hefs_gplot

ggsave(paste('e:/Projects/FIRO/firo_syn-forecast_stochastic-CC/figs/ind_plots/hefs-skillmod-ens-plot_evt=',evt_order,'_mods=',str_flatten(skill_mods,collapse='-'),'_ld=',lead,'.png',sep=''),hefs_gplot,dpi=320,width=9,height=3,unit='in')
...
show_x = F
labs = letters[(length(skill_mods)+1):(length(skill_mods)*2)]
...
show_x = T
labs = letters[(length(skill_mods)*3+1):(length(skill_mods)*4)]
ylm = 75

for(l in 1:length(lds)){
  shefs_mat <- array(NA,c(dim(shefs_ecrps_sset)[2]*dim(shefs_ecrps_sset)[3],length(skill_mods)))
  shefscc_mat <- array(NA,c(dim(shefscc_ecrps_sset)[2]*dim(shefscc_ecrps_sset)[3],length(skill_mods)))
  hefs_mat <- array(NA,c(dim(hefs_ecrps_sset)[3],length(skill_mods)))
  for(i in 1:length(skill_mods)){
    shefs_mat[,i]<-as.vector(shefs_ecrps_sset[i,l,])
    shefscc_mat[,i]<-as.vector(shefscc_ecrps_sset[i,l,])
    hefs_mat[,i]<-hefs_ecrps_sset[i,l,]
  }
  ...
  ecrps_bplt <- ggplot(df)+theme_minimal()+
    ...
    coord_cartesian(ylim = c(0,ylm))+
    labs(title=paste('ld',lds[l]))+
    annotate('text',x=0,y=0.95*ylm,label=paste(labs[l],')',sep=''),size=6,hjust=0)+
    theme(axis.text=element_text(size=10),
          axis.title=element_text(size=12),
...
ggsave(paste('e:/Projects/FIRO/firo_syn-forecast_stochastic-CC/figs/ind_plots/',loc,'-',disp_site,'_ecrps-skillmod-bplots_pcntile=',disp_pcnt,'_lds=',str_flatten(lds,collapse='-'),'_mod=',str_flatten(skill_mods,collapse='-'),'.png',sep=''),ecrps_gplot,dpi=320,width=2.5*length(lds),height=3,unit='in')
...
layout_mat <- matrix(1:(4*length(lds)),nrow=4,ncol=length(lds),byrow=T)
comb_plot<-marrangeGrob(c(hefs_plts,shefs_plts,shefscc_plts,ecrps_plts),nrow=4,ncol=length(lds),top='',layout_matrix = layout_mat, widths = c(1.15,1,1,1),heights = c(1,1,1.1,1.25))

ggsave(paste('e:/Projects/FIRO/firo_syn-forecast_stochastic-CC/figs/forecast_plots/combined_ens-plot_verification-plot_skillmods_fig5_evt=',evt_order,'_nn=',nn_order,'_lds=',str_flatten(leads,collapse='-'),'_samp=',samp_no,'_mods=',str_flatten(skill_mods,'-'),'.png',sep=''),comb_plot,dpi=320,width=9,height=11,unit='in')
